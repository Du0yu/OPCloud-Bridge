...
刷新同一标签页后，点击“恢复备份”恢复上次模型，或点击“保留当前模型”用当前画布替换旧备份。选择前暂停自动保存。普通独立标签页使用各自的备份标识；复制标签页可能继承该标识。备份位于当前浏览器，关闭标签页后重新新建标签页不保证找回原标识，因此重要模型仍应导出 `.opcl`。存储被禁用或空间不足时面板会显示错误，内存中的撤回仍可使用。自动保存不会下载文件或上传到服务器。
